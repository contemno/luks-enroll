...
#     its minor/major to cut a minor/major release. Falls back to
#     rust/service/Cargo.toml's version when ./VERSION is absent.
...
if [ -f VERSION ]; then
    ...
    floor="$(grep -m1 -E '^version[[:space:]]*=' rust/service/Cargo.toml \
