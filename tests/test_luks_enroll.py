...
class TestSyntax(unittest.TestCase):
    ...
    def test_service_syntax(self):
        with open(SERVICE_PATH) as f:
            ast.parse(f.read(), filename=SERVICE_PATH)
...
class TestProxyServiceConsistency(unittest.TestCase):
    ...
    def test_proxy_calls_match_xml_methods(self):
        ...
        # Parse the GUI source to find all .call_sync("Name" and .call("Name"
        with open(GUI_PATH) as f:
            source = f.read()
...
class TestConstants(unittest.TestCase):
    def test_bus_name(self):
        self.assertEqual(svc.BUS_NAME, "net.contemno.LuksEnroll")

    def test_object_path(self):
        self.assertEqual(svc.OBJECT_PATH, "/net/contemno/LuksEnroll")

    def test_interface_name(self):
        self.assertEqual(svc.INTERFACE_NAME, "net.contemno.LuksEnroll1")

    def test_polkit_actions(self):
        self.assertEqual(svc.POLKIT_ACTION_READ, "net.contemno.luks-enroll.read")
        self.assertEqual(svc.POLKIT_ACTION_MANAGE, "net.contemno.luks-enroll.manage")
